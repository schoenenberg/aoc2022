...
pub fn solve(path: &str) -> Result<(String, String), Box<dyn std::error::Error>> {
    ...
    for l in lines {
        ...
        if a.completely_contains(&b) || b.completely_contains(&a) {
            contains_counter += 1;
        }
        if a.overlap(&b) {
            overlap_counter += 1;
    ...
    Ok((contains_counter.to_string(), overlap_counter.to_string()))
}
...
impl Range {
    fn completely_contains(&self, b: &Self) -> bool {
        self.low <= b.low && self.high >= b.high
    }

    fn overlap(&self, b: &Self) -> bool {
        self.low <= b.high && b.low <= self.high
    }
}