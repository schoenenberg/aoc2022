...
pub fn solve(path: &str) -> Result<(String, String), Box<dyn std::error::Error>> {
    ...
    for (i, l) in lines.enumerate() {
        let chars: HashSet<char> = l.chars().collect();
        rucksacks.push(chars);

        if i != 0 && (i + 1) % 3 == 0 {
            for a in rucksacks[i].iter() {
                if rucksacks[i - 1].contains(a) && rucksacks[i - 2].contains(a) {
                    badge_sum += char_value(a);
                }
            }
        }
    }

    Ok((sum.to_string(), badge_sum.to_string()))
}
...
mod test {
    #[test]
    fn char_value() {
        assert_eq!(super::char_value(&'a'), 1);
        assert_eq!(super::char_value(&'z'), 26);
        assert_eq!(super::char_value(&'A'), 27);
        assert_eq!(super::char_value(&'Z'), 52);
    }
}